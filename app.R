...
shinyApp(
    ...
    server = function(input, output, session) {
        ...
        # functions because of potential overlap with `shiny` functions.
        ...
        observeEvent(input$orthoSearch, {
            if (nrow(mappedOrthoGenes()) == 0 & nrow(missingOrthoGenes()) == 0) {
                insertAlert(
                    ...
                    type = "danger",
                    content = paste0(
                        "ERROR: No matches were found for your genes. Please ",
                        "check your inputs and ensure they are in the correct ",
                        "format."
                    )
                )
            }
